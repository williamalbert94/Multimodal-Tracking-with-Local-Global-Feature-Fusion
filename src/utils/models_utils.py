...
import os
import random
from utils.track_kitti_3d import load_detection
...
def euc_distance(p1, p2):
    # p1 and p2 are already tensors, no need to wrap with torch.tensor()
    return (p1 - p2).pow(2).sum(dim=1).sqrt()
...
def filter_object_points(args, labels, pc, transforms):
    ...
    boxes = dict()
    boxes_centre = dict()
    class_labels = dict()  # NEW: Store class labels for each object
    for key, label in labels.items():
        ...
        class_labels[label.id] = label.type  # NEW: Store class label (e.g., 'car', 'pedestrian', 'cyclist')
    ...
    for obj_id, box in boxes.items():
        ...
        objs[obj_id] = pc[:, :, in_box_idx].clone()
        objs_centre[obj_id] = obj_centre(objs[obj_id])
        objs_idx[obj_id] = torch.as_tensor(in_box_idx, dtype=torch.long, device='cuda')
        if len(pc_fil) == 0:
            pc_fil = pc[:, :, in_box_idx].clone()
        else:
            pc_fil = torch.cat((pc_fil, pc[:, :, in_box_idx].clone()), dim=2)
    ...
    class_labels_combined = dict()  # NEW: Class labels for valid objects
    for obj_id, obj in objs.items():
        if obj_id not in ids_to_pop:
            ...
            class_labels_combined[obj_id] = class_labels.get(obj_id, 'unknown')  # NEW: Store class label

    if len(pc_fil) == 0:
        pc_fil = None

    return pc_fil, cls, objs, objs_idx, objs_centre, cls_obj_id, boxes, objs_combined, objs_idx_combined, objs_centre_combined, class_labels_combined
...
def get_gt_flow_new(obj_c1, obj_c2, obj_cls1, obj_cls_id1, obj_cls_id2, pc1, pc1_comp, bboxes1, bboxes2):
    # pc1: [1, 3, N], pc1_comp: [1, 3, N]
    # Returns [3, N] - target positions in frame 2 (NOT flow deltas)
    # NOTE: Despite the name "flow", this returns POSITIONS not DELTAS

    # Remove batch dimension for easier processing
    pc1_unbatched = pc1.squeeze(0)  # [3, N]
    pc1_comp_unbatched = pc1_comp.squeeze(0)  # [3, N]

    gt_flow_pc = []
    for i in range(pc1_unbatched.size(1)):  # Iterate over N points
        in_box = False
        if obj_cls1[i]:
            if int(obj_cls_id1[i]) in obj_c2.keys():
                ...
                # Get point as [3] vector
                pnt = pc1_unbatched[:, i]  # [3]

                # Transform point from frame1 to frame2 using object's rigid transformation
                # Convert to homogeneous coordinates [4]
                pnt_hom = torch.cat([pnt, torch.ones(1).cuda()])  # [4]
                gt_p = (torch.tensor(t_bbx1_bbx2, dtype=torch.float32).cuda() @ pnt_hom)[:3]  # [3]

                # Return POSITION (not delta) - this is what the original code did
                gt_flow_pc.append(gt_p.unsqueeze(1))  # [3, 1]
                in_box = True

        if not in_box:
            # For static points: return ego-compensated position
            gt_flow_pc.append(pc1_comp_unbatched[:, i].unsqueeze(1))  # [3, 1]

    return torch.cat(gt_flow_pc, dim=1)  # Concatenate along dim=1 to get [3, N]
...
def filter_moving_boxes_det(raw_detection_labels, lbls):
    ...
    for i in range(len(raw_detection_labels)):
        ...
        # Filter: only moving objects (all classes: vehicles, pedestrians, cyclists, motorcyclists)
        obj = lbls[key]
        if mov == 1:
...
def filter_object_points_batch(args, labels_batch, pc_batch, transforms_batch):
    """
    Batch version of filter_object_points.

    Args:
        args: arguments with min_obj_points configuration
        labels_batch: list of label dictionaries, one per batch element
        pc_batch: tensor of shape [B, 3, N]
        transforms_batch: list of FrameTransformMatrix, one per batch element

    Returns:
        Lists of outputs, one element per batch item:
        - gt_mov_pts_batch: list of filtered point tensors (or None)
        - gt_cls_batch: list of classification masks
        - gt_objs_batch: list of object dictionaries
        - objs_idx_batch: list of object index dictionaries
        - objs_centre_batch: list of object center dictionaries
        - cls_obj_id_batch: list of object ID per point tensors
        - boxes_batch: list of bounding box dictionaries
        - objs_combined_batch: list of combined object dictionaries
        - objs_idx_combined_batch: list of combined index dictionaries
        - objs_centre_combined_batch: list of combined center dictionaries
        - class_labels_batch: list of class label dictionaries {obj_id: class_name}
    """
    ...
    boxes_batch = []
    objs_combined_batch = []
    objs_idx_combined_batch = []
    objs_centre_combined_batch = []
    class_labels_batch = []  # NEW: Class labels for each batch

    # Process each batch element independently
    for b in range(batch_size):
        # Extract single element: [3, N]
        pc_single = pc_batch[b:b+1]  # Keep batch dimension as [1, 3, N]
        labels_single = labels_batch[b]
        transforms_single = transforms_batch[b]

        # Call original filter_object_points function
        gt_mov_pts, gt_cls, gt_objs, objs_idx, objs_centre, cls_obj_id, boxes, objs_combined, objs_idx_combined, objs_centre_combined, class_labels_combined = filter_object_points(
            args, labels_single, pc_single, transforms_single
        )

        # Append to batch lists
        gt_mov_pts_batch.append(gt_mov_pts)
        gt_cls_batch.append(gt_cls)
        gt_objs_batch.append(gt_objs)
        objs_idx_batch.append(objs_idx)
        objs_centre_batch.append(objs_centre)
        cls_obj_id_batch.append(cls_obj_id)
        boxes_batch.append(boxes)
        objs_combined_batch.append(objs_combined)
        objs_idx_combined_batch.append(objs_idx_combined)
        objs_centre_combined_batch.append(objs_centre_combined)
        class_labels_batch.append(class_labels_combined)  # NEW: Store class labels

    return (gt_mov_pts_batch, gt_cls_batch, gt_objs_batch, objs_idx_batch,
            objs_centre_batch, cls_obj_id_batch, boxes_batch, objs_combined_batch,
            objs_idx_combined_batch, objs_centre_combined_batch, class_labels_batch)


def get_gt_flow_new_batch(objs_centre1_batch, objs_centre2_batch, gt_cls1_batch,
                          cls_obj_id1_batch, cls_obj_id2_batch, pc1_batch,
                          pc1_compensated_batch, boxes1_batch, boxes2_batch):
    """
    Batch version of get_gt_flow_new.

    Computes ground truth scene flow for each point:
    - Dynamic points (in moving objects): transformed by object's rigid motion
    - Static points: compensated by ego-motion only

    Args:
        objs_centre1_batch: list of dicts with object centers (frame 1)
        objs_centre2_batch: list of dicts with object centers (frame 2)
        gt_cls1_batch: list of classification masks [N]
        cls_obj_id1_batch: list of object ID per point [N]
        cls_obj_id2_batch: list of object ID per point [N] (not used in original)
        pc1_batch: tensor [B, 3, N]
        pc1_compensated_batch: tensor [B, 3, N]
        boxes1_batch: list of dicts with bounding boxes (frame 1)
        boxes2_batch: list of dicts with bounding boxes (frame 2)

    Returns:
        gt_flow_batch: list of tensors [1, 3, N], one per batch element
    """
    ...
    for b in range(batch_size):
        ...
        obj_c1 = objs_centre1_batch[b]
        obj_c2 = objs_centre2_batch[b]
        obj_cls1 = gt_cls1_batch[b]
        cls_obj_id1 = cls_obj_id1_batch[b]
        pc1 = pc1_batch[b:b+1]  # [1, 3, N]
        pc1_comp = pc1_compensated_batch[b:b+1]  # [1, 3, N]
        bboxes1 = boxes1_batch[b]
        bboxes2 = boxes2_batch[b]

        # Call original get_gt_flow_new function
        # Note: passing cls_obj_id1 twice because cls_obj_id2 is not actually used in the function
        gt_flow = get_gt_flow_new(obj_c1, obj_c2, obj_cls1, cls_obj_id1, cls_obj_id1,
                                  pc1, pc1_comp, bboxes1, bboxes2)

        gt_flow_batch.append(gt_flow)

    return gt_flow_batch